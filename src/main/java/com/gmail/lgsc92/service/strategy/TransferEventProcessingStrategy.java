...
@Component
public class TransferEventProcessingStrategy implements EventProcessingStrategy {

    private final AccountService accountService;
    private final EventMapper eventMapper;
    private final Validator validator;

    public TransferEventProcessingStrategy(AccountService accountService, EventMapper eventMapper) {
    ...
    public EventOutputDTO process(EventRequestDTO genericDTO) {
        TransferEventInputDTO transferDTO = eventMapper.toTransfer(genericDTO);
        Set<ConstraintViolation<TransferEventInputDTO>> violations = validator.validate(transferDTO);
        if (!violations.isEmpty()) {
            throw new ConstraintViolationException(violations);
        }
        return accountService.processTransfer(transferDTO);
    }
}
